...
8. **Phase 4.18 Continuous Read-Only Shadow Detection Validation [FACT / EMPIRICAL]**:
   ...
   - **Strict Population Separation [AUDITED RULE]**:
     - **Population A (Continuous Campaign)**: $N=292$ candidate paths evaluated. 0 survived the local economic gate. 0 RPC verification calls were sent during the continuous loop. 292 candidate evaluations were rejected by off-chain friction gates before dispatching an RPC query (100% avoidance ratio within this calm market sample).
     - **Population B (Standalone Benchmark)**: On live Base Mainnet (Block 51441353), local predicted output vs. on-chain `QuoterV2` for a 1.0 WETH swap ($ts=50$ tick spacing, fee=500) produced identical outputs (`2454007702` atomic units each), demonstrating **$\Delta = 0$ wei ($0.0000$ bps drift)** $\to$ classified as `EXACT` match for this specific state.
...
9. **Phase 4.18.1 Forensic Correction, Economic Accounting & Evidence Audit [AUDITED]**:
   - **Fee Accounting Invariant [PROVEN RULE]**: DEX swap fees (5 bps V3, 30 bps V2) are embedded in AMM swap outputs. They are not separately deducted in net expected PnL, confirming zero fee double-counting.
   - **Bidirectional Cash-Flow Integrity [FACT]**: Corrected CEX-DEX gross PnL in `evaluateCexDex` to calculate actual proceeds: $((N_{\text{out}} / 10^{18}) \cdot P_{\text{CEX}}) - N_{\text{USD}}$ for `DEX_TO_CEX`, and $(N_{\text{out}} / 10^6) - ((Q_{\text{in}} \cdot P_{\text{CEX}}) / 10^{18})$ for `CEX_TO_DEX`.
   - **Terminology Invariant [ENFORCED RULE]**: Renamed simulated candidate lifecycle state to `SHADOW_SIMULATED` (`SHADOW_SIMULATED ≠ ACTUAL EXECUTION`).
   - **Model Assumption Boundaries [EXPLICIT]**: CEX transfer costs ($1.00 USD), withdrawal times (120 s), and inventory rebalancing are classified as `[MODEL ASSUMPTION]`. Cross-venue clock alignment is classified as `UNKNOWN`.
10. **Phase 5 Validation (Pending Operator Gate)**: Develop atomic on-chain arbitrage contract (`ArbitrageExecutor.sol`) with strict post-swap balance checks and atomic revert guarantees. Execution remains strictly LOCKED (₹0 capital, 0 private keys).
